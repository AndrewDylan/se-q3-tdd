...
class TestEcho(unittest.TestCase):
    ...
    def test_title_short(self):
        """Check if short option '-t' titles the string"""
        args = ["-t", "hello"]
        output = run_capture(self.module.__file__, args)
        self.assertEqual(output[0], "Hello")

    def test_title_long(self):
        """Check if long option '--title' titles the string"""
        args = ["--title", "hello"]
        output = run_capture(self.module.__file__, args)
        self.assertEqual(output[0], "Hello")

    def test_multiple_options(self):
        """Check if multiple options '-tul' are applied in order"""
        args = ["-tul", "heLLo!"]
        output = run_capture(self.module.__file__, args)
        self.assertEqual(output[0], "hello!")
    ...
    def test_help_message(self):
        ...
        args = ["-h"]
        # output = run_capture(self.module.__file__, args)
        # Could not figure out how to test program would just exit
        # Tried capturing the Exception it through but no luck
        self.assertEqual(run_capture(self.module.__file__, args), "hello!")
    ...
    def test_author(self):
        ...
        self.assertNotEqual(
            __author__, "???",
            "Test author string is not completed"
            )
